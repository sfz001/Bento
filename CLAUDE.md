...
4. **Menu bar icon manager (菜单栏图标管理)** — Bartender/Ice-style selective hide/show of third-party menu bar icons: menu opens a checklist window listing every third-party icon by owning app; unchecking drags it (synthetic ⌘-drag) behind a 10000pt-wide "hider" status item so macOS parks it offscreen; checking drags it back. Bento's own icon is pinned rightmost among third-party icons. Hidden keys persist in UserDefaults (`HiddenMenuBarItemKeys`). System modules (clock/battery/…) are left to System Settings → Control Center.
